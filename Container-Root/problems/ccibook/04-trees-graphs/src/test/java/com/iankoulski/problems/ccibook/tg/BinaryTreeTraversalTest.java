package com.iankoulski.problems.ccibook.tg;

import org.junit.Test;
import org.junit.Assert;
import org.junit.runners.model.TestClass;

/**
 * Unit test for simple App.
 */
public class BinaryTreeTraversalTest extends TestClass
{

    public BinaryTreeTraversalTest()
    {
        super(BinaryTreeTraversalTest.class);
        btt = new BinaryTreeTraversal();
    }

    private BinaryTreeTraversal btt;

    // test main
    @Test
    public void testApp()
    {   
        System.out.println("App test:");
        com.iankoulski.problems.ccibook.tg.BinaryTreeTraversal.main(null);
        Assert.assertTrue(true);
    }

    public BinaryTreeNode<Integer> getTestTree(boolean balanced){
        /*
        balanced == true

                 1
               /   \
              2     3
             / \   / 
            4   5 6   


        balanced == false
                 1
               /   \
              2     3
             /     / 
            4     6   
           /
          5

        */
        BinaryTreeNode<Integer> one = new BinaryTreeNode<Integer>(1);
        BinaryTreeNode<Integer> two = new BinaryTreeNode<Integer>(2);
        BinaryTreeNode<Integer> three = new BinaryTreeNode<Integer>(3);
        BinaryTreeNode<Integer> four = new BinaryTreeNode<Integer>(4);
        BinaryTreeNode<Integer> five = new BinaryTreeNode<Integer>(5);
        BinaryTreeNode<Integer> six = new BinaryTreeNode<Integer>(6);
        if (balanced){
            one.left=two;
            one.right=three;
            two.left=four;
            two.right=five;
            three.left=six;
            four.parent=two;
            five.parent=two;
            two.parent=one;
            six.parent=three;
            three.parent=one;
        }else{
            one.left=two;
            one.right=three;
            three.left=six;
            two.left=four;
            four.left=five;
            five.parent=four;
            four.parent=two;
            six.parent=three;
            three.parent=one;
        }
        return one;
    }

    @Test
    public void testBlanacedSuccessorOfFive()
    {
        BinaryTreeNode<Integer> tree = getTestTree(true);
        Integer successor = btt.getSuccessor(tree.left.right, TraversalMode.InOrder);
        Assert.assertTrue(successor.intValue() == 1 );
    }

    @Test
    public void testUnbalancedSuccessorOfThree()
    {
        BinaryTreeNode<Integer> tree = getTestTree(false);
        Integer successor = btt.getSuccessor(tree.right, TraversalMode.InOrder);
        Assert.assertTrue(successor == null);
    }

}
